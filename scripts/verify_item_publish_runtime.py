...
_LEGACY_RECONCILIATION_PROBLEM_CODE_DIAGNOSTICS = {
...
def _legacy_collection_diagnostics_enabled() -> bool:
    return bool(
        ...
        or LEGACY_POST_P809_FINAL_GATE_DIAGNOSTICS_ENABLED
    )
...
def legacy_reconciliation_failure_message(
    ...
    if diagnostic is not None:
        ...
        if result.status != 409:
            ...
            code = (
                _LEGACY_RECONCILIATION_PROBLEM_CODE_DIAGNOSTICS.get(
                    problem_code,
                    "P803_LEGACY_RECONCILIATION_BODY_CODE",
                )
                if isinstance(problem_code, str)
                else "P803_LEGACY_RECONCILIATION_BODY_CODE"
            )
