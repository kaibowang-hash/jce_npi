...
class NPIERPToolAssetMapping(Document):
    def autoname(self) -> None:
        self.source_stream_key_hash = _hash(
            self.source_stream_key_hash,
            _("Tool Asset Source Stream Key Hash"),
        )
    ...
    def validate(self) -> None:
        ...
        if previous is not None:
            for fieldname in (
                ...
                if getattr(previous, fieldname, None) != getattr(
                    self,
                    fieldname,
                    None,
                ):
                    frappe.throw(
                        _("ERPNext Tool Asset mapping identity fields are immutable."),
        ...
        self.project_global_id = _uuid(
            self.project_global_id,
            _("Project Global ID"),
        )
        self.tooling_set_global_id = _uuid(
            self.tooling_set_global_id,
            _("Tooling Set Global ID"),
        )
        ...
        if type(self.mapping_version) is not int or self.mapping_version != expected_version:
            frappe.throw(
                _("The ERPNext Tool Asset mapping version must advance by one."),
                frappe.ValidationError,
            )
