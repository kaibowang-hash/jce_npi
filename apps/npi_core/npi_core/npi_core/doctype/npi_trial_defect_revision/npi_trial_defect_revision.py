...
from npi_core.trial.quality_metadata_validation import (
    normalize_trial_defect_identity,
    validate_trial_defect_document,
)


class NPITrialDefectRevision(Document):
    """Immutable Trial-bound successor in one NPI defect timeline."""
    ...
    def before_validate(self) -> None:
        normalize_trial_defect_identity(self)

    def validate(self) -> None:
        if self.get_doc_before_save() is not None:
            deny_trial_history_update()
        validate_trial_defect_document(self)
