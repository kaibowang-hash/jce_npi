...
from npi_core.trial.review_metadata_validation import (
    normalize_conclusion_identity,
    validate_conclusion_document,
)


class NPITrialConclusionRevision(Document):
    """Immutable Trial conclusion submit, decision or reopen revision."""
    ...
    def before_validate(self) -> None:
        normalize_conclusion_identity(self)

    def validate(self) -> None:
        if self.get_doc_before_save() is not None:
            deny_trial_history_update()
        validate_conclusion_document(self)
