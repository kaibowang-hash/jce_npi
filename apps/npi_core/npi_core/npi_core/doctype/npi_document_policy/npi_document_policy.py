...
_IDENTITY_FIELDS = ("global_id", "tenant_id", "policy_key", "policy_key_hash")


class NPIDocumentPolicy(Document):
    """Administrative availability root; policy rules live in immutable versions."""
    ...
    def before_validate(self) -> None:
        ...
        self.policy_key = key_text(self.policy_key, _("Policy Key"))
        self.policy_key_hash = hashlib.sha256(
            f"{self.tenant_id}:{self.policy_key.casefold()}".encode("utf-8")
        ).hexdigest()
        self.title = required_text(self.title, _("Title"), 140)
        if self.is_new():
