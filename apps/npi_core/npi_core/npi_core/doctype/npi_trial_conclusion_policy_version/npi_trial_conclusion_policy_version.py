...
from npi_core.trial.review_metadata_validation import (
    normalize_policy_identity,
    validate_policy_document,
)


class NPITrialConclusionPolicyVersion(Document):
    """Immutable exact Trial conclusion policy version."""
    ...
    def before_validate(self) -> None:
        normalize_policy_identity(self)

    def validate(self) -> None:
        if self.get_doc_before_save() is not None:
            deny_trial_history_update()
        validate_policy_document(self)
