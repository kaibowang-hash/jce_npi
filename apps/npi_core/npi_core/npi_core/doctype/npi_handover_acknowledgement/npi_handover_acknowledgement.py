...
from npi_core.production_transition.metadata_validation import (
    normalize_handover_acknowledgement_identity,
    validate_handover_acknowledgement_document,
)


class NPIHandoverAcknowledgement(Document):
    """Immutable actor-bound acknowledgement of one exact package slot."""

    def autoname(self) -> None:
        normalize_handover_acknowledgement_identity(self)
    ...
    def validate(self) -> None:
        if self.get_doc_before_save() is not None:
            deny_production_transition_history_update()
        validate_handover_acknowledgement_document(self)
