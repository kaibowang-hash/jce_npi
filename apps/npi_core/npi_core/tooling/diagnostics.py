...
TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_CODES = frozenset(
    {
...
TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_HEADER = "X-NPI-Diagnostic-Scope"
TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_SCOPE = "p603-revision-create-v1"
_DIAGNOSTIC_FLAG = "npi_p601_part_create_diagnostic"
_APPLICABILITY_DIAGNOSTIC_FLAG = "npi_p601_applicability_create_diagnostic"
_TOOLING_REVISION_DIAGNOSTIC_FLAG = "npi_p603_revision_create_diagnostic"
...
@contextmanager
def tooling_revision_create_server_diagnostics(
    trace_id: str | None,
    *,
    route_enabled: bool,
) -> Iterator[None]:
    """Enable one closed, response-neutral P6-03 create diagnostic."""

    try:
        enabled = route_enabled and (
            frappe.get_request_header(
                TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_HEADER
            )
            == TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_SCOPE
        )
        state = None
        if enabled and isinstance(trace_id, str) and _TRACE_PATTERN.fullmatch(trace_id):
            state = {"trace_id": trace_id, "recorded": False}
        flags = frappe.flags
        missing = object()
        previous = getattr(flags, _TOOLING_REVISION_DIAGNOSTIC_FLAG, missing)
        setattr(flags, _TOOLING_REVISION_DIAGNOSTIC_FLAG, state)
    except Exception:
        yield
        return
    try:
        ...
        try:
            if previous is missing:
                delattr(flags, _TOOLING_REVISION_DIAGNOSTIC_FLAG)
            else:
                setattr(flags, _TOOLING_REVISION_DIAGNOSTIC_FLAG, previous)
        except Exception:
            pass


@contextmanager
def tooling_revision_create_server_step(code: str) -> Iterator[None]:
    """Record one allowlisted Tooling Revision create substage unchanged."""

    try:
        yield
    except Exception as error:
        _record_tooling_revision_create_server_failure(code, error)
        raise
...
def _record_tooling_revision_create_server_failure(
    code: str,
    error: Exception,
) -> None:
    """Record only the revision stage, exception type and exact trace."""

    try:
        state = _tooling_revision_diagnostic_state()
        exception_type = type(error).__name__
        if (
            state is None
            or state["recorded"] is True
            or code not in TOOLING_REVISION_CREATE_SERVER_DIAGNOSTIC_CODES
            or _TYPE_PATTERN.fullmatch(exception_type) is None
        ):
            return
        state["recorded"] = True
...
def _tooling_revision_diagnostic_state() -> dict[str, object] | None:
    state = getattr(frappe.flags, _TOOLING_REVISION_DIAGNOSTIC_FLAG, None)
    if (
        not isinstance(state, dict)
        or set(state) != {"trace_id", "recorded"}
        or not isinstance(state.get("trace_id"), str)
