        ...
        frappe.get_doc(
            {
                ...
                "comparison_snapshot_global_id": str(value.comparison_snapshot.global_id),
                "comparison_snapshot_hash": value.comparison_snapshot.snapshot_hash,
                "review_reference_snapshot": canonical_json(payload["reviewReferences"]),
                "blocker_snapshot": canonical_json(payload["blockers"]),
                "summary_input_snapshot": canonical_json(value.summary_input),
                "proposed_next_work_snapshot": canonical_json(list(value.proposed_next_work)),
                "proposed_gate_effect": value.proposed_gate_effect,
                "proposed_npi_effect": value.proposed_npi_effect,
                "external_effect_snapshot": canonical_json(payload["externalEffects"]),
                "reason": value.reason,
                "created_by_user_id": value.created_by_user_id,
                "created_at": _database_datetime(value.created_at),
                "request_id": str(value.request_id),
                "trace_id": value.trace_id,
                "conclusion_snapshot": canonical_json(payload),
                "snapshot_hash": value.snapshot_hash,
            }
        ).insert()
