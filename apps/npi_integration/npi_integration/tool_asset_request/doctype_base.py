...
class ToolAssetSupportDocument(Document):
    """Fail-closed base for dormant Tool Asset execution support metadata."""
    ...
    def autoname(self) -> None:
        label = self.meta.get_label(self.identity_field)
        identity = lowercase_sha256(getattr(self, self.identity_field), label) if self.identity_is_hash else canonical_uuid(getattr(self, self.identity_field), label)
    ...
    def validate(self) -> None:
        for fieldname in self.uuid_fields:
            setattr(self, fieldname, canonical_uuid(getattr(self, fieldname), self.meta.get_label(fieldname)))
        for fieldname in self.optional_uuid_fields:
            if getattr(self, fieldname, None):
                setattr(self, fieldname, canonical_uuid(getattr(self, fieldname), self.meta.get_label(fieldname)))
        for fieldname in self.hash_fields:
            setattr(self, fieldname, lowercase_sha256(getattr(self, fieldname), self.meta.get_label(fieldname)))
        for fieldname in self.optional_hash_fields:
            if getattr(self, fieldname, None):
                setattr(self, fieldname, lowercase_sha256(getattr(self, fieldname), self.meta.get_label(fieldname)))
        for fieldname in self.positive_fields:
            setattr(self, fieldname, positive_integer(getattr(self, fieldname), self.meta.get_label(fieldname)))
        for fieldname in self.nonnegative_fields:
            setattr(self, fieldname, nonnegative_integer(getattr(self, fieldname), self.meta.get_label(fieldname)))
        ...
        for fieldname in self.required_text_fields:
            setattr(self, fieldname, required_text(getattr(self, fieldname), self.meta.get_label(fieldname), 280))
        for fieldname in self.optional_text_fields:
            if getattr(self, fieldname, None):
                setattr(self, fieldname, required_text(getattr(self, fieldname), self.meta.get_label(fieldname), 280))
        for fieldname in self.actor_fields:
            setattr(self, fieldname, actor_text(getattr(self, fieldname), self.meta.get_label(fieldname)))
    ...
    def _require_write(self, action: str) -> None:
        if self.write_guard is None:
            deny_tool_asset_execution_history_update()
        self.write_guard()
        require_tool_asset_execution_capability(self.doctype, action)
