...
from .diagnostics import (
    suppress_tool_asset_process_stage_diagnostics,
    tool_asset_process_stage_step,
)
...
def process_outbox_message(outbox_event_id: str) -> dict[str, object]:
    ...
    with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RESPONSE_BUILD"):
        if outcome is None:
...
def _execute_worker(*, outbox_event_id: UUID, repository, profile_resolver: Callable[[str, UUID], ToolAssetExecutionProfile | None], registry_resolver: Callable[[], ToolAssetAdapterRegistry | None], clock: Callable[[], datetime]) -> ToolAssetWorkerOutcome | None:
    try:
        ...
        return None
    try:
        with tool_asset_process_stage_step(
            "P805_TOOL_ASSET_PROCESS_ACTOR_SCOPE",
            ignored=(ToolAssetServiceActorUnavailable,),
        ), tool_asset_service_actor_scope(route.service_actor_user_id):
            with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_CLAIM"):
                claim = repository.claim(outbox_event_id, now=clock(), expected_route=route)
            if claim is None:
                _rollback_safely()
                return None
            with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_CLAIM_COMMIT"):
                _commit_or_raise("TOOL_ASSET_CLAIM_COMMIT_FAILED", claim.trace_id)
            if claim.recovered_after_adapter_boundary:
                with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RECOVERED_RESULT"):
                ...
                with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RECOVERED_PERSIST"):
            ...
            try:
                ...
                _rollback_safely()
                return _persist_failed_before_boundary(
                    repository,
                    claim,
                ...
                return _persist_failed_before_boundary(
                    repository,
                    claim,
                    safe_error_code="TOOL_ASSET_EXECUTION_CONFIGURATION_INVALID",
                    clock=clock,
                )
            with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_BOUNDARY"):
                boundary_sealed = repository.mark_adapter_boundary(claim, profile=profile, now=clock())
            if not boundary_sealed:
                _rollback_safely()
                raise RuntimeError("Tool Asset adapter boundary could not be sealed safely.")
            with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_BOUNDARY_COMMIT"):
            ...
            try:
                ...
                with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_UNCERTAIN_RESULT"):
                    result = uncertain_tool_asset_adapter_result(command=claim.command, safe_error_code="TOOL_ASSET_ADAPTER_OUTCOME_UNCERTAIN")
            with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RESULT_PERSIST"):
...
def _persist_failed_before_boundary(
    repository,
    claim,
    *,
    safe_error_code: str,
    ...
    with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_FAILURE_RESULT"):
        result = failed_before_tool_asset_adapter_boundary_result(
            command=claim.command,
            safe_error_code=safe_error_code,
        )
    with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_FAILURE_PERSIST"):
        return _persist(repository, claim, profile=None, result=result, now=clock())
...
def _persist(repository, claim, *, profile, result, now):
    committed = False
    try:
        with suppress_tool_asset_process_stage_diagnostics():
            outcome = repository.seal_result(claim, profile=profile, result=result, now=now)
        committed = True
        with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RESULT_COMMIT"):
        ...
        if not callable(recover):
            raise
        with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RESULT_RECOVERY"):
            outcome = recover(claim, profile=profile, result=result, now=now)
        with tool_asset_process_stage_step("P805_TOOL_ASSET_PROCESS_RECOVERY_COMMIT"):
