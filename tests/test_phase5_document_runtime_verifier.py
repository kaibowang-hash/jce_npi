...
class Phase5DocumentRuntimeVerifierTest(unittest.TestCase):
    ...
    def test_baseline_workspace_http_server_substage_is_closed_and_preferred(
        self,
    ) -> None:
        module = self.module
        expected_codes = {
        ...
        self.assertEqual(
            set(module._BASELINE_WORKSPACE_SERVER_DIAGNOSTIC_CODES),
            expected_codes,
        )
        trace_id = "trace-" + ("e" * 32)
        code = "P503_BASELINE_WORKSPACE_PROJECT_MEMBERSHIP_QUERY"
        policy_hash = "a" * 64
            ...
            (log_directory / "npi_core.log").write_text(
                "\n".join(
                    (
                        ...
                        "request cookie=controlled-secret",
                    )
                ),
                encoding="utf-8",
            )
            with (
                ...
                module.validate_initial_document_baseline_workspace(
                    module.HttpResult(
                        ...
                        headers=Mock(),
                        body={
                            "message": "raw response must not be emitted",
                            "cookie": "controlled-secret",
                        },
                        trace_id=trace_id,
                    ),
                    expected_policy_hash=policy_hash,
        ...
        self.assertEqual(failure.exception.exception_type, "DatabaseError")
        self.assertEqual(failure.exception.trace_id, trace_id)
    ...
    def test_baseline_workspace_server_diagnostic_scope_is_initial_query_only(
        self,
    ) -> None:
        module = self.module
        result = Mock()
        with patch.object(module, "npi_request", return_value=result) as request:
            self.assertIs(
                module.document_baseline_request(
                    Mock(),
                    "http://127.0.0.1:8003",
                    module.fixture_id("project"),
                    query_key="baseline-empty",
                ),
                result,
            )
            self.assertEqual(
                request.call_args.kwargs["server_diagnostic_scope"],
                module._BASELINE_WORKSPACE_SERVER_DIAGNOSTIC_SCOPE,
            )
