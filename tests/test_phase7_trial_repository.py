...
class Phase7TrialRepositoryTest(unittest.TestCase):
    MODULES = (
        ...
        "npi_core.trial.execution_repository",
    )
    ...
    def setUp(self) -> None:
        ...
        self.execution_module = importlib.import_module(
            "npi_core.trial.execution_repository"
        )
        ...
        self.execution_repository = (
            self.execution_module.FrappeTrialExecutionRepository(
                principal=self.principal,
                request_id=REQUEST_ID,
                trace_id="trace-trial-execution-repository-001",
            )
        )
    ...
    def seed_execution_round(self) -> tuple[UUID, dict[str, object]]:
        ...
        outcome = self.repository.create_round(
            ...
            display_label=None,
            reason="Create the planned execution Round.",
        )
        round_response = outcome.response["rounds"][0]
        return UUID(round_response["globalId"]), round_response

    def seed_execution_references(self) -> tuple[list[dict[str, object]], tuple[UUID, UUID]]:
        baseline_id = uuid4()
        part_revision_id = uuid4()
        tooling_revision_id = uuid4()
        tooling_set_id = uuid4()
        binding_id = uuid4()
        cavity_ids = (uuid4(), uuid4())
        process_chain_id = uuid4()
        inspection_document_id = uuid4()
        controlled_document_id = uuid4()
        ...
        self.add(
            ...
            {
                "global_id": str(tooling_revision_id),
                "tenant_id": "TENANT-A",
                "project_global_id": str(PROJECT_ID),
                "tooling_master_global_id": str(MASTER_ID),
                "revision_number": 3,
                "cavity_snapshot": [
                    {"globalId": str(cavity_ids[0]), "label": "C01"},
                    {"globalId": str(cavity_ids[1]), "label": "C02"},
                ],
                "snapshot_hash": "d" * 64,
            },
        )
        ...
        self.add(
            "NPI Tooling Set Revision Binding",
            {
                ...
                "tooling_set_snapshot_hash": "e" * 64,
                "tooling_revision_global_id": str(tooling_revision_id),
        ...
        self.add(
            ...
            {
                "global_id": str(process_chain_id),
                "tenant_id": "TENANT-A",
                "project_global_id": str(PROJECT_ID),
                "chain_version": 4,
                "snapshot_hash": "1" * 64,
        ...
        self.add(
            ...
            {
                "global_id": str(inspection_document_id),
                "tenant_id": "TENANT-A",
                "project_global_id": str(PROJECT_ID),
        ...
        self.add(
            ...
            {
                "global_id": str(inspection_document_id),
                "revision_global_id": str(inspection_document_id),
                "tenant_id": "TENANT-A",
                "project_global_id": str(PROJECT_ID),
                "document_global_id": str(controlled_document_id),
        ...
        references = [
            {"globalId": baseline_id, "kind": "design_baseline", "expectedOptimisticVersion": 1},
            {"globalId": part_revision_id, "kind": "part_revision", "expectedOptimisticVersion": 2},
            {"globalId": tooling_revision_id, "kind": "tooling_revision", "expectedOptimisticVersion": 3},
            {"globalId": tooling_set_id, "kind": "tooling_set", "expectedOptimisticVersion": 1},
            {"globalId": binding_id, "kind": "tooling_set_binding", "expectedOptimisticVersion": 1},
            {"globalId": cavity_ids[0], "kind": "cavity", "expectedOptimisticVersion": 3},
            {"globalId": cavity_ids[1], "kind": "cavity", "expectedOptimisticVersion": 3},
            {"globalId": process_chain_id, "kind": "process_chain", "expectedOptimisticVersion": 4},
            {"globalId": inspection_document_id, "kind": "inspection_document", "expectedOptimisticVersion": 5},
        ]
        return references, cavity_ids

    @staticmethod
    def execution_material() -> dict[str, object]:
        return {
            "sourceSystem": "NPI_ONE",
            "sourceObjectId": "material-observation-1",
            "lotBatchCode": "lot-2026-08-10",
            "label": "PA66 natural",
            "color": None,
            "additive": None,
            "observedAt": datetime(2026, 8, 10, 8, tzinfo=UTC),
        }

    @staticmethod
    def actual_context() -> dict[str, object]:
        return {
            "resources": (
                {
                    "kind": "machine",
                    "sourceSystem": "NPI_ONE",
                    "sourceObjectId": "machine-160t-1",
                    "label": "160T machine",
                },
            ),
            "material": Phase7TrialRepositoryTest.execution_material(),
            "environment": (),
            "parameters": (
                {
                    ...
                    "unit": "degC",
                    "source": "manual",
                    "observedAt": datetime(2026, 8, 10, 9, tzinfo=UTC),
                },
            ),
            "operator_user_id": "operator@example.invalid",
            "execution_started_at": datetime(2026, 8, 10, 9, tzinfo=UTC),
            "reason": "Record the exact manual execution context.",
        }
    ...
    def test_execution_prepare_start_actual_and_sample_are_exact_and_append_only(self) -> None:
        round_id, round_response = self.seed_execution_round()
        references, cavity_ids = self.seed_execution_references()
        prepared = self.execution_repository.prepare_round(
            PROJECT_ID,
            round_id,
            ...
            reason="Freeze the exact Trial execution inputs.",
        )
        self.assertEqual(prepared.response["round"]["currentState"], "prepared")
        self.assertEqual(len(prepared.response["inputLocks"][0]["references"]), 9)
        self.assertEqual(
            sum(
                item["kind"] == "cavity"
                for item in prepared.response["inputLocks"][0]["references"]
            ),
            2,
        )
        replay = self.execution_repository.prepare_round(
            PROJECT_ID,
            round_id,
            idempotency_key_hash="8" * 64,
            expected_round_optimistic_version=round_response["optimisticVersion"],
        ...
        input_lock = prepared.response["inputLocks"][0]
        started = self.execution_repository.start_round(
            PROJECT_ID,
            round_id,
            idempotency_key_hash="9" * 64,
            expected_round_optimistic_version=2,
            expected_input_lock_revision_global_id=UUID(input_lock["globalId"]),
            expected_input_lock_version=1,
            **self.actual_context(),
        )
        self.assertEqual(started.response["round"]["currentState"], "running")
        self.assertEqual(started.response["actualRevisions"][0]["actualVersion"], 1)

        first_actual = started.response["actualRevisions"][0]
        revised_actual = self.execution_repository.append_actual_revision(
            PROJECT_ID,
            round_id,
            idempotency_key_hash="a" * 64,
            expected_round_optimistic_version=3,
            expected_actual_revision_global_id=UUID(first_actual["globalId"]),
            expected_actual_version=1,
            **{
                **self.actual_context(),
                "reason": "Append one exact manual Actual successor.",
            },
        )
        self.assertEqual(len(revised_actual.response["actualRevisions"]), 2)
        self.assertEqual(
            revised_actual.response["actualRevisions"][-1]["predecessorGlobalId"],
            first_actual["globalId"],
        )
        ...
        sample = {
            ...
            "quantity": 80,
            "unit": "piece",
            "packaging": "Two sealed trays",
            "destination": "Dimensional laboratory",
            "feedbackText": None,
            "feedbackSource": None,
            "feedbackObservedAt": None,
        }
        created_sample = self.execution_repository.create_sample_batch(
            PROJECT_ID,
            round_id,
            ...
            expected_input_lock_revision_global_id=UUID(input_lock["globalId"]),
            sample=sample,
            reason="Create the exact Sample Batch.",
        )
        first_sample = created_sample.response["sampleBatchRevisions"][0]
        revised_sample = self.execution_repository.append_sample_batch_revision(
            PROJECT_ID,
            round_id,
            UUID(first_sample["sampleBatchGlobalId"]),
            idempotency_key_hash="c" * 64,
            expected_round_optimistic_version=3,
            expected_revision_global_id=UUID(first_sample["globalId"]),
            expected_sample_version=1,
            sample={**sample, "packaging": "Two sealed labelled trays"},
            reason="Append the corrected packaging truth.",
        )
        self.assertEqual(len(revised_sample.response["sampleBatchRevisions"]), 2)
        self.assertEqual(
            revised_sample.response["sampleBatchRevisions"][-1]["sampleVersion"],
            2,
        )

        evidence_bytes = b"%PDF-1.7\ntrial evidence\n"
        file_revision_id = uuid4()
        file_document_id = "trial-evidence-file-1"
        self.add(
            "NPI File Revision",
            {
        ...
        snapshot = {
            "globalId": str(file_revision_id),
            "fileId": file_document_id,
            "fileName": "trial-report.pdf",
            "mimeType": "application/pdf",
            "sizeBytes": len(evidence_bytes),
            "sha256": hashlib.sha256(evidence_bytes).hexdigest(),
            "fileContentHash": hashlib.md5(
                evidence_bytes,
        ...
        evidence_domain = importlib.import_module("npi_core.trial.execution_domain")
        with (
            patch.object(
            ...
            bound = self.execution_repository.bind_evidence(
                ...
                expected_round_optimistic_version=3,
                role=evidence_domain.TrialEvidenceRole.MEASUREMENT_REPORT,
                file_revision_global_id=file_revision_id,
                expected_file_optimistic_version=2,
                sample_batch_revision_global_id=UUID(
                    revised_sample.response["sampleBatchRevisions"][-1]["globalId"]
                ),
                expected_sample_version=2,
            )
            self.assertEqual(len(bound.response["evidence"]), 1)
            serialized = json.dumps(bound.response, sort_keys=True)
            self.assertNotIn("fileUrl", serialized)
            self.assertNotIn("/private/files/", serialized)
            evidence_id = UUID(bound.response["evidence"][0]["globalId"])
            content = self.execution_repository.evidence_content(
                PROJECT_ID,
                round_id,
                evidence_id,
            )
        self.assertEqual(content.content, evidence_bytes)
        self.assertEqual(content.mime_type, "application/pdf")
        access_events = [
            document
            for document in self.documents["NPI Audit Event"].values()
            if document.operation == "trial_evidence.content.read"
        ]
        self.assertEqual(len(access_events), 1)

    def test_prepare_rejects_superseded_inspection_document(self) -> None:
        round_id, round_response = self.seed_execution_round()
        references, _cavity_ids = self.seed_execution_references()
        inspection_reference = next(
            item for item in references if item["kind"] == "inspection_document"
        )
        lifecycle = self.documents["NPI Document Revision Lifecycle"][
            str(inspection_reference["globalId"])
        ]
        lifecycle.current_state = "superseded"

        with self.assertRaises(
            self.execution_module.TrialExecutionReferenceUnavailable
        ):
            self.execution_repository.prepare_round(
                PROJECT_ID,
                round_id,
                idempotency_key_hash="e" * 64,
                expected_round_optimistic_version=round_response[
                    "optimisticVersion"
                ],
                references=references,
                material=self.execution_material(),
                ...
                reason="Reject a superseded inspection document.",
            )

    def test_upload_bytes_are_not_read_before_project_authorization(self) -> None:
        security = importlib.import_module("npi_core.foundation.security")
        unauthorized_repository = (
            self.execution_module.FrappeTrialExecutionRepository(
                ...
                request_id=REQUEST_ID,
                trace_id="trace-trial-execution-unauthorized-001",
            )
        )
        upload_read = False

        def uploaded_file() -> tuple[str, bytes]:
            nonlocal upload_read
            upload_read = True
            return "trial-report.pdf", b"%PDF-1.7\ncontrolled evidence\n"

        result = unauthorized_repository.upload_evidence_file(
            PROJECT_ID,
            ROUND_ID,
        ...
        self.assertFalse(upload_read)

    def test_trial_upload_observation_rejects_extension_content_mismatch(self) -> None:
        observation = self.execution_module._observe_trial_upload(
            "trial-report.pdf",
            b"%PDF-1.7\ncontrolled evidence\n",
        )
        self.assertEqual(observation.mime_type, "application/pdf")
        self.assertEqual(len(observation.sha256), 64)
        errors = importlib.import_module("npi_core.foundation.errors")
        with self.assertRaises(errors.RequestValidationFailed):
            self.execution_module._observe_trial_upload(
                "forged.pdf",
                b"not a PDF",
            )
